...
class InterfaceTableGenerator:
    ...
    def construct_kernel_interface_table(self):
        ...
        while True:
            if 0 == self.check_network_interface():
                ...
                time.sleep(1)
        # 进行 名称 -> 链路标识的映射的遍历
        final_str = f"{len(self.name_to_lid_map)}\n"
        ...
        self.netlink_client.send_netlink_data(final_str, ncm.NetlinkMessageType.CMD_CONSTRUCT_NEW_INTERFACE_TABLE)
    ...
    def start(self):
        ...
        :return:
        """
        print(f"-------------start generate lir interface table-------------", flush=True)
