...
class Node:
    class Type(Enum):
        ...
        def __str__(self) -> Union[str, ValueError]:
            ...
            :return:
            """
            if self.value == self.SATELLITE_NODE.value:
                return "SATELLITE_NODE"
            elif self.value == self.GROUND_STATION_NODE.value:
                return "GROUND_STATION_NODE"
            else:
    ...
    def resolve_container_name(self) -> str:
        if self.node_type.value == Node.Type.SATELLITE_NODE.value:
            container_name = f"satellite-node{self.node_id}"
        elif self.node_type.value == Node.Type.GROUND_STATION_NODE.value:
            container_name = f"normal-node{self.node_id}"
        else:
